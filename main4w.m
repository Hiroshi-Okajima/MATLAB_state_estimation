...
gammaoptimal = 100000;
gammaoptimal1 = 100000;
gammaoptimal2 = 100000;
gammaoptimal3 = 100000;
gammaoptimal4 = 100000;
...
S0 = newlmi;
lmiterm([-S0 1 1 P],(1-alpha0),1)
lmiterm([-S0 3 1 P],1,A0-L0p*C)
lmiterm([-S0 3 2 P],1,B0-L0p*D0)
lmiterm([-S0 2 2 0],alpha0)
lmiterm([-S0 3 3 P],1,1)
...
[copt1,xopt1] = mincx(LMIs,c);

if isempty(xopt1)
else
Popt1 = dec2mat(LMIs,xopt1,P);
gammaopt21 = dec2mat(LMIs,xopt1,gamma);
...
if (gammaopt21 < gammaoptimal1)&&(alpha0<1-al0^2)&&(alpha1<1-al1^2)&&(alpha2<1-al2^2)&&(alpha3<1-al3^2)
    gammaoptimal1 = gammaopt21;
    iopti = i;
    jopti = j;
    kopti = k;
    lopti = l;
...
S0 = newlmi;
lmiterm([-S0 1 1 P],(1-alpha1),1)
lmiterm([-S0 3 1 P],1,A1-L1p*C)
lmiterm([-S0 3 2 P],1,B1-L1p*D1)
lmiterm([-S0 2 2 0],alpha1)
lmiterm([-S0 3 3 P],1,1)
...
[copt2,xopt2] = mincx(LMIs,c);

if isempty(xopt2)
else
Popt2 = dec2mat(LMIs,xopt2,P);
gammaopt22 = dec2mat(LMIs,xopt2,gamma);
...
if (gammaopt22 < gammaoptimal2)&&(alpha0<1-al0^2)&&(alpha1<1-al1^2)&&(alpha2<1-al2^2)&&(alpha3<1-al3^2)
    gammaoptimal2 = gammaopt22;
    iopti = i;
    jopti = j;
    kopti = k;
    lopti = l;
...
S0 = newlmi;
lmiterm([-S0 1 1 P],(1-alpha2),1)
lmiterm([-S0 3 1 P],1,A2-L2p*C)
lmiterm([-S0 3 2 P],1,B2-L2p*D2)
lmiterm([-S0 2 2 0],alpha2)
lmiterm([-S0 3 3 P],1,1)
...
[copt3,xopt3] = mincx(LMIs,c);

if isempty(xopt3)
else
Popt3 = dec2mat(LMIs,xopt3,P);
gammaopt23 = dec2mat(LMIs,xopt3,gamma);
...
if (gammaopt23 < gammaoptimal3)&&(alpha0<1-al0^2)&&(alpha1<1-al1^2)&&(alpha2<1-al2^2)&&(alpha3<1-al3^2)
    gammaoptimal3 = gammaopt23;
    iopti = i;
    jopti = j;
    kopti = k;
    lopti = l;
...
S0 = newlmi;
lmiterm([-S0 1 1 P],(1-alpha3),1)
lmiterm([-S0 3 1 P],1,A3-L3p*C)
lmiterm([-S0 3 2 P],1,B3-L3p*D3)
lmiterm([-S0 2 2 0],alpha3)
lmiterm([-S0 3 3 P],1,1)
...
[copt4,xopt4] = mincx(LMIs,c);

if isempty(xopt4)
else
Popt4 = dec2mat(LMIs,xopt4,P);
gammaopt24 = dec2mat(LMIs,xopt4,gamma);
...
if (gammaopt24 < gammaoptimal4)&&(alpha0<1-al0^2)&&(alpha1<1-al1^2)&&(alpha2<1-al2^2)&&(alpha3<1-al3^2)
    gammaoptimal4 = gammaopt24;
    iopti = i;
    jopti = j;
    kopti = k;
    lopti = l;
...
hold on;
            ...
            F(3,1) =  [1 1] * ya(:,k) - [1 1] * C * xtotal(:,k-1) + [1 1] * C * Bu * u(:,k-1);
            F(4,1) =  [1 1] * ya(:,k) - [1 1] * C * xtotal(:,k-1) + [1 1] * C * Bu * u(:,k-1);
            F(5,1) = [1 1] * ya(:,k-1) - [1 1] * C * xtotal(:,k-2) + [1 1] * C * Bu * u(:,k-2);
            F(6,1) = [1 1] * ya(:,k-1) - [1 1] * C * xtotal(:,k-2) + [1 1] * C * Bu * u(:,k-2);
            F(7,1) =  [1 1] * ya(:,k-1) - [1 1] * C * xtotal(:,k-2) + [1 1] * C * Bu * u(:,k-2);
            F(8,1) =  [1 1] * ya(:,k-2) - [1 1] * C * xtotal(:,k-3) + [1 1] * C * Bu * u(:,k-3);
            F(9,1) = [1 1] * ya(:,k-3) - [1 1] * C * xtotal(:,k-4) + [1 1] * C * Bu * u(:,k-4);
            [I1,I2] = sort(F);
...
figure(5),plot(F2,'ok'),axis([0 600 -0.5 3.5])
